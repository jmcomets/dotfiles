...
namespace Bundle
{
        ...
        class Vector
    {
        public:
            ...
            //
            // @return the index'th coordinate
            const T & Coord(std::size_t index) const
            {
                if (index >= N)
                {
                    throw std::out_of_range("Given index is out of bounds");
    ...
    template <class T>
        class Vector<T, 3> // FIXME
    {
        public:
            Vector Cross(const Vector & v)
            {
                Vector cross;
                cross[0] = _coords[1] * v._coords[2] - _coords[2] * v._coords[1];
                cross[1] = _coords[2] * v._coords[0] - _coords[0] * v._coords[2];
                cross[2] = _coords[0] * v._coords[1] - _coords[1] * v._coords[0];
                return cross;
            }
    };
